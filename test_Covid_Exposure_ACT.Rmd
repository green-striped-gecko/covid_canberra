...
#m
bscols(
m <- leaflet(sd) %>% addTiles() %>%  addCircleMarkers(lat =tt$lat,lng = tt$lon, popup = tt$labs, weight=0.5, color = tt$cols, radius = 5 , fillOpacity = 0.8) %>% 
 addLegend("bottomright", labels = levels(factor(tt$Contact)), colors = cols)
)
